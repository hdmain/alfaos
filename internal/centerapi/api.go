...
func (s *Server) handleQuality(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		writeJSON(w, http.StatusOK, map[string]any{
			...
			"applied": false,
			"warning": err.Error(),
			"hint":    "Saved on host. Reconnect RDP once. Display size can stay the same.",
		})
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"bpp":     qualityBPP(quality),
		"applied": applied,
		"hint":    "RDP profile saved. Disconnect and reconnect once. Display resolution may stay unchanged.",
	})
}
...
func applyGuestQuality(cfg *config.Config, quality string, w, h int) (bool, error) {
	...
	if !vm.DomainExists() || !vm.DomainRunning() {
		return false, fmt.Errorf("VM not running — profile saved for next session")
	}
